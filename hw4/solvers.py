...
def optimizeWithIP(x0, A, b, C, d, mu=1.0, rate=0.1, tol=1e-6, max_iter=1000):
    ...
    F[id_v] = np.diag(v).dot(d - C.dot(x)) - mu*np.ones(k)
    ...
    # *************************************************************************
    # TODO: Fill in dF
    dF[id_x, id_x] = A.T.dot(A)
    dF[id_x, id_v] = C.T
    dF[id_v, id_x] = -np.diag(v).dot(C)
    dF[id_v, id_v] = np.diag(d - C.dot(x))
    # Hint: it is easier to use id_x and id_v to access different components
    # *************************************************************************
    #
    ...
    while err >= tol:
        ...
        ind = np.where(C.dot(dx) > 0.0)[0]
        alpha = min(alpha, 0.99 * np.min(((d - C.dot(x))/C.dot(dx))[ind]))
        # *********************************************************************
...
def optimizeWithCP(x0, A, b, c, h, k, prox_ch, prox_k,
    ...
    while err >= tol:
        ...
        x_new = prox_k(x + alpha*A.T.dot(v)- alpha*c, alpha)
        rx_new = b - A.dot(x_new)
        # *********************************************************************
        # update v
        # *********************************************************************
        # TODO: update v and rv
        v_new = prox_ch(v + alpha*A.dot(x - 2*x_new) + alpha*b, alpha)
